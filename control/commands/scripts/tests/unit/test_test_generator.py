...
class TestTestConfigGenerator(unittest.TestCase):
    ...
    def test_init(self):
        ...
        generator = TestConfigGenerator("config.toml")
        self.assertEqual(generator.config_path, Path("config.toml"))
        self.assertEqual(generator.project_root, Path("config.toml").parent)
