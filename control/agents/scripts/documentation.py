#!/usr/bin/env python3
"""
Documentation subagent generator script.

This script parses the TOML configuration file and generates the appropriate
documentation subagent configuration based on the settings. It validates the configuration
...
class DocumentationAgentGenerator:
    """Generates documentation subagent configuration from TOML settings."""
    ...
    def validate_documentation_config(self, config: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """
        Validate the documentation configuration and return the agent settings.

        Returns None if the configuration should not generate anything.
        """
        # The documentation config is under opencode.agents.subagents
        opencode_config = config.get('opencode', {})
        agents_config = opencode_config.get('agents', {})
        subagents_config = agents_config.get('subagents', {})
        documentation_config = subagents_config.get('documentation', {})

        if not documentation_config:
            print("No documentation configuration found in TOML file")
            return None

        # Check if enabled is false
        if not documentation_config.get('enabled', True):
            print("Documentation subagent is disabled (enabled = false)")
            return None

        return documentation_config

    def validate_and_read_template(self, documentation_config: Dict[str, Any]) -> str:
        """
        ...
        """
        template_type = documentation_config.get('template', 'default')
        template_file = documentation_config.get('template_file', '')
        additional_files = documentation_config.get('additional_files', [])
        additional_files_strategy = documentation_config.get('additional_files_strategy', 'merge')
    ...
    def _get_output_path(self, documentation_config: Dict[str, Any]) -> str:
        """
        Determine the output path based on the agent mode.

        Subagents output to '.opencode/agent/subagent/documentation.md'
        """
        return "generated/.opencode/agent/subagent/documentation.md"

    def _extract_agent_config(self, documentation_config: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    def write_agent_config(self, documentation_config: Dict[str, Any], template_content: str) -> None:
        """Write the agent configuration to a Markdown file with YAML frontmatter."""
        output_path = self._get_output_path(documentation_config)
        ...
        # Extract agent configuration (excluding config keys)
        agent_config = self._extract_agent_config(documentation_config)
        ...
        print(f"Generated documentation subagent configuration: {output_file}")
    ...
    def generate(self) -> bool:
        ...
        try:
            ...
            documentation_config = self.validate_documentation_config(config)
            if documentation_config is None:
                return False

            # Validate template and read content
            template_content = self.validate_and_read_template(documentation_config)

            # Write the agent configuration file
            self.write_agent_config(documentation_config, template_content)

            return True

        except Exception as e:
            print(f"Error generating documentation subagent configuration: {e}", file=sys.stderr)
            sys.exit(1)
...
def main():
    ...
    generator = DocumentationAgentGenerator(config_path)
    success = generator.generate()

    if success:
        print("Documentation subagent configuration generated successfully")
    else:
        print("Documentation subagent configuration generation skipped")
