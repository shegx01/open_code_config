#!/usr/bin/env python3
"""
Reviewer subagent generator script.

This script parses the TOML configuration file and generates the appropriate
reviewer subagent configuration based on the settings. It validates the configuration
...
class ReviewerAgentGenerator:
    """Generates reviewer subagent configuration from TOML settings."""
    ...
    def validate_reviewer_config(self, config: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """
        Validate the reviewer configuration and return the agent settings.

        Returns None if the configuration should not generate anything.
        """
        # The reviewer config is under opencode.agents.subagents
        opencode_config = config.get('opencode', {})
        agents_config = opencode_config.get('agents', {})
        subagents_config = agents_config.get('subagents', {})
        reviewer_config = subagents_config.get('reviewer', {})

        if not reviewer_config:
            print("No reviewer configuration found in TOML file")
            return None

        # Check if enabled is false
        if not reviewer_config.get('enabled', True):
            print("Reviewer subagent is disabled (enabled = false)")
            return None

        return reviewer_config

    def validate_and_read_template(self, reviewer_config: Dict[str, Any]) -> str:
        """
        ...
        """
        template_type = reviewer_config.get('template', 'default')
        template_file = reviewer_config.get('template_file', '')
        additional_files = reviewer_config.get('additional_files', [])
        additional_files_strategy = reviewer_config.get('additional_files_strategy', 'merge')
        ...
        if additional_files_strategy != 'replace' or not additional_files:
            if template_type == 'default':
                # Use default template location for reviewer subagent
                default_template_path = self.project_root / "control/agents/subagents/reviewer.md"
    ...
    def _get_output_path(self, reviewer_config: Dict[str, Any]) -> str:
        """
        Determine the output path based on the agent mode.

        Subagents output to '.opencode/agent/subagent/reviewer.md'
        """
        return "generated/.opencode/agent/subagent/reviewer.md"

    def _extract_agent_config(self, reviewer_config: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        agent_config = {}
        for key, value in reviewer_config.items():
    ...
    def write_agent_config(self, reviewer_config: Dict[str, Any], template_content: str) -> None:
        """Write the agent configuration to a Markdown file with YAML frontmatter."""
        output_path = self._get_output_path(reviewer_config)
        ...
        # Extract agent configuration (excluding config keys)
        agent_config = self._extract_agent_config(reviewer_config)
        ...
        print(f"Generated reviewer subagent configuration: {output_file}")
    ...
    def generate(self) -> bool:
        ...
        try:
            ...
            reviewer_config = self.validate_reviewer_config(config)
            if reviewer_config is None:
                return False

            # Validate template and read content
            template_content = self.validate_and_read_template(reviewer_config)

            # Write the agent configuration file
            self.write_agent_config(reviewer_config, template_content)

            return True

        except Exception as e:
            print(f"Error generating reviewer subagent configuration: {e}", file=sys.stderr)
            sys.exit(1)
...
def main():
    ...
    generator = ReviewerAgentGenerator(config_path)
    success = generator.generate()

    if success:
        print("Reviewer subagent configuration generated successfully")
    else:
        print("Reviewer subagent configuration generation skipped")
