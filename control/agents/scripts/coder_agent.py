#!/usr/bin/env python3
"""
Coder-agent subagent generator script.

This script parses the TOML configuration file and generates the appropriate
coder-agent subagent configuration based on the settings. It validates the configuration
...
class CoderAgentGenerator:
    """Generates coder-agent subagent configuration from TOML settings."""
    ...
    def validate_coder_agent_config(self, config: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """
        Validate the coder-agent configuration and return the agent settings.

        Returns None if the configuration should not generate anything.
        """
        # The coder-agent config is under opencode.agents.subagents
        opencode_config = config.get('opencode', {})
        agents_config = opencode_config.get('agents', {})
        subagents_config = agents_config.get('subagents', {})
        coder_agent_config = subagents_config.get('coder-agent', {})

        if not coder_agent_config:
            print("No coder-agent configuration found in TOML file")
            return None

        # Check if enabled is false
        if not coder_agent_config.get('enabled', True):
            print("Coder-agent subagent is disabled (enabled = false)")
            return None

        return coder_agent_config

    def validate_and_read_template(self, coder_agent_config: Dict[str, Any]) -> str:
        """
        ...
        """
        template_type = coder_agent_config.get('template', 'default')
        template_file = coder_agent_config.get('template_file', '')
        additional_files = coder_agent_config.get('additional_files', [])
        additional_files_strategy = coder_agent_config.get('additional_files_strategy', 'merge')
        include_base_template = coder_agent_config.get('include_base_template', True)
        lang = coder_agent_config.get('lang', '')
        ...
        if additional_files_strategy != 'replace' or not additional_files:
            if template_type == 'default':
                ...
                if include_base_template:
                    base_template_path = self.project_root / "control/agents/subagents/coder-agent/base.md"
                ...
                if lang:
                    lang_template_path = self.project_root / f"control/agents/subagents/coder-agent/{lang}.md"
    ...
    def _get_output_path(self, coder_agent_config: Dict[str, Any]) -> str:
        """
        Determine the output path based on the agent mode.

        Subagents output to '.opencode/agent/subagent/coder-agent.md'
        """
        return "generated/.opencode/agent/subagent/coder-agent.md"

    def _extract_agent_config(self, coder_agent_config: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        agent_config = {}
        for key, value in coder_agent_config.items():
    ...
    def write_agent_config(self, coder_agent_config: Dict[str, Any], template_content: str) -> None:
        """Write the agent configuration to a Markdown file with YAML frontmatter."""
        output_path = self._get_output_path(coder_agent_config)
        ...
        # Extract agent configuration (excluding config keys)
        agent_config = self._extract_agent_config(coder_agent_config)
        ...
        print(f"Generated coder-agent subagent configuration: {output_file}")
    ...
    def generate(self) -> bool:
        ...
        try:
            ...
            coder_agent_config = self.validate_coder_agent_config(config)
            if coder_agent_config is None:
                return False

            # Validate template and read content
            template_content = self.validate_and_read_template(coder_agent_config)

            # Write the agent configuration file
            self.write_agent_config(coder_agent_config, template_content)

            return True

        except Exception as e:
            print(f"Error generating coder-agent subagent configuration: {e}", file=sys.stderr)
            sys.exit(1)
...
def main():
    ...
    generator = CoderAgentGenerator(config_path)
    success = generator.generate()

    if success:
        print("Coder-agent subagent configuration generated successfully")
    else:
        print("Coder-agent subagent configuration generation skipped")
