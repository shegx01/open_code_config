#!/usr/bin/env python3
"""
Codebase-agent generator script.

This script parses the TOML configuration file and generates the appropriate
codebase-agent configuration based on the settings. It validates the configuration
...
class CodebaseAgentGenerator:
    """Generates codebase-agent configuration from TOML settings."""
    ...
    def validate_codebase_agent_config(self, config: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """
        Validate the codebase-agent configuration and return the agent settings.

        Returns None if the configuration should not generate anything.
        """
        # The codebase-agent config is under opencode.agents
        opencode_config = config.get('opencode', {})
        agents_config = opencode_config.get('agents', {})
        codebase_agent_config = agents_config.get('codebase-agent', {})

        if not codebase_agent_config:
            print("No codebase-agent configuration found in TOML file")
            return None

        # Check if enabled is false
        if not codebase_agent_config.get('enabled', True):
            print("Codebase-agent is disabled (enabled = false)")
            return None

        return codebase_agent_config

    def validate_and_read_template(self, codebase_agent_config: Dict[str, Any]) -> str:
        """
        ...
        """
        template_type = codebase_agent_config.get('template', 'default')
        template_file = codebase_agent_config.get('template_file', '')
        additional_files = codebase_agent_config.get('additional_files', [])
        additional_files_strategy = codebase_agent_config.get('additional_files_strategy', 'merge')
        ...
        if additional_files_strategy != 'replace' or not additional_files:
            if template_type == 'default':
                # Use default template location for codebase-agent
                default_template_path = self.project_root / "control/agents/codebase-agent.md"
    ...
    def _get_output_path(self, codebase_agent_config: Dict[str, Any]) -> str:
        """
        Determine the output path based on the agent mode.

        Primary agents output to '.opencode/agent/codebase-agent.md'
        """
        return "generated/.opencode/agent/codebase-agent.md"

    def _extract_agent_config(self, codebase_agent_config: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        agent_config = {}
        for key, value in codebase_agent_config.items():
    ...
    def write_agent_config(self, codebase_agent_config: Dict[str, Any], template_content: str) -> None:
        """Write the agent configuration to a Markdown file with YAML frontmatter."""
        output_path = self._get_output_path(codebase_agent_config)
        ...
        # Extract agent configuration (excluding config keys)
        agent_config = self._extract_agent_config(codebase_agent_config)
        ...
        print(f"Generated codebase-agent configuration: {output_file}")
    ...
    def generate(self) -> bool:
        ...
        try:
            ...
            # Validate codebase-agent configuration
            codebase_agent_config = self.validate_codebase_agent_config(config)
            if codebase_agent_config is None:
                return False

            # Validate template and read content
            template_content = self.validate_and_read_template(codebase_agent_config)

            # Write the agent configuration file
            self.write_agent_config(codebase_agent_config, template_content)

            return True

        except Exception as e:
            print(f"Error generating codebase-agent configuration: {e}", file=sys.stderr)
            sys.exit(1)
...
def main():
    ...
    generator = CodebaseAgentGenerator(config_path)
    success = generator.generate()

    if success:
        print("Codebase-agent configuration generated successfully")
    else:
        print("Codebase-agent configuration generation skipped")
