#!/usr/bin/env python3
"""
Code-pattern-analyst subagent generator script.

This script parses the TOML configuration file and generates the appropriate
code-pattern-analyst subagent configuration based on the settings. It validates the configuration
...
class CodePatternAnalystAgentGenerator:
    """Generates code-pattern-analyst subagent configuration from TOML settings."""
    ...
    def validate_code_pattern_analyst_config(self, config: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """
        Validate the code-pattern-analyst configuration and return the agent settings.

        Returns None if the configuration should not generate anything.
        """
        # The code-pattern-analyst config is under opencode.agents.subagents
        opencode_config = config.get('opencode', {})
        agents_config = opencode_config.get('agents', {})
        subagents_config = agents_config.get('subagents', {})
        code_pattern_analyst_config = subagents_config.get('code-pattern-analyst', {})

        if not code_pattern_analyst_config:
            print("No code-pattern-analyst configuration found in TOML file")
            return None

        # Check if enabled is false
        if not code_pattern_analyst_config.get('enabled', True):
            print("Code-pattern-analyst subagent is disabled (enabled = false)")
            return None

        return code_pattern_analyst_config

    def validate_and_read_template(self, code_pattern_analyst_config: Dict[str, Any]) -> str:
        """
        ...
        """
        template_type = code_pattern_analyst_config.get('template', 'default')
        template_file = code_pattern_analyst_config.get('template_file', '')
        additional_files = code_pattern_analyst_config.get('additional_files', [])
        additional_files_strategy = code_pattern_analyst_config.get('additional_files_strategy', 'merge')
        include_base_template = code_pattern_analyst_config.get('include_base_template', True)
        lang = code_pattern_analyst_config.get('lang', '')
        ...
        if additional_files_strategy != 'replace' or not additional_files:
            if template_type == 'default':
                ...
                if include_base_template:
                    base_template_path = self.project_root / "control/agents/subagents/code-pattern-analyst/base.md"
                ...
                if lang:
                    lang_template_path = self.project_root / f"control/agents/subagents/code-pattern-analyst/{lang}.md"
                    if not lang_template_path.exists():
                        raise FileNotFoundError(f"Language template not found: {lang_template_path}")

                    # Only include language template if it has content
                    if lang_template_path.stat().st_size > 0:
                        try:
                            with open(lang_template_path, 'r', encoding='utf-8') as f:
                                lang_content = f.read()
                                if lang_content.strip():  # Only add if content is not just whitespace
                                    template_content_parts.append(f"\n\n# {lang.title()} Pattern Analysis Specifics\n\n{lang_content}")
                        except Exception as e:
    ...
    def _get_output_path(self, code_pattern_analyst_config: Dict[str, Any]) -> str:
        """
        Determine the output path based on the agent mode.

        Subagents output to '.opencode/agent/subagent/code-pattern-analyst.md'
        """
        return "generated/.opencode/agent/subagent/code-pattern-analyst.md"

    def _extract_agent_config(self, code_pattern_analyst_config: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        agent_config = {}
        for key, value in code_pattern_analyst_config.items():
    ...
    def write_agent_config(self, code_pattern_analyst_config: Dict[str, Any], template_content: str) -> None:
        """Write the agent configuration to a Markdown file with YAML frontmatter."""
        output_path = self._get_output_path(code_pattern_analyst_config)
        ...
        # Extract agent configuration (excluding config keys)
        agent_config = self._extract_agent_config(code_pattern_analyst_config)
        ...
        print(f"Generated code-pattern-analyst subagent configuration: {output_file}")
    ...
    def generate(self) -> bool:
        ...
        try:
            ...
            # Validate code-pattern-analyst configuration
            code_pattern_analyst_config = self.validate_code_pattern_analyst_config(config)
            if code_pattern_analyst_config is None:
                return False

            # Validate template and read content
            template_content = self.validate_and_read_template(code_pattern_analyst_config)

            # Write the agent configuration file
            self.write_agent_config(code_pattern_analyst_config, template_content)

            return True

        except Exception as e:
            print(f"Error generating code-pattern-analyst subagent configuration: {e}", file=sys.stderr)
            sys.exit(1)
...
def main():
    ...
    generator = CodePatternAnalystAgentGenerator(config_path)
    success = generator.generate()

    if success:
        print("Code-pattern-analyst subagent configuration generated successfully")
    else:
        print("Code-pattern-analyst subagent configuration generation skipped")
