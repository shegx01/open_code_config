#!/usr/bin/env python3
"""
Clean command generator script.

This script parses the TOML configuration file and generates the appropriate
clean command configuration based on the settings. It validates the configuration
and generates a Markdown file with YAML frontmatter matching the clean command format.
"""
...
class CleanConfigGenerator:
    """Generates clean command configuration from TOML settings."""
    ...
    def validate_clean_config(self, config: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """
        Validate the clean configuration and return the clean settings.
    ...
    def validate_and_read_template(self, clean_config: Dict[str, Any]) -> str:
        ...
        if additional_files_strategy != 'replace' or not additional_files:
            if template_type == 'default':
                # For clean command, we need to handle base template and language-specific templates
                template_content_parts = []
                
                # Include base template if requested
                if include_base_template:
                    base_template_path = self.project_root / "ogc/control/commands/generic/clean/base.md"
                ...
                if lang:
                    lang_template_path = self.project_root / f"ogc/control/commands/generic/clean/{lang}.md"
    ...
    def generate_yaml_config(self, clean_config: Dict[str, Any], template_content: str) -> Dict[str, Any]:
        """Generate the YAML configuration matching the clean.yaml format."""
        return {
    ...
    def write_yaml_config(self, clean_config: Dict[str, Any], template_content: str, output_path: str = "ogc/generated/.opencode/command/clean.md") -> None:
