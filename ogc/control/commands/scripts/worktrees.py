#!/usr/bin/env python3
"""
Worktrees command generator script.

This script parses the TOML configuration file and generates the appropriate
worktrees command configuration based on the settings. It validates the configuration
and generates a JSON file matching the worktrees.jsonc template format.
"""
...
class WorktreesConfigGenerator:
    """Generates worktrees command configuration from TOML settings."""
    ...
    def validate_worktrees_config(self, config: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """
        Validate the worktrees configuration and return the worktrees settings.
        ...
        worktrees_config = commands_config.get('worktrees', {})
        
        if not worktrees_config:
            print("No worktrees configuration found in TOML file")
            return None
            
        # Check if enabled is false
        if not worktrees_config.get('enabled', True):
            print("Worktrees command is disabled (enabled = false)")
            return None
            
        # Check if vcs is not git
        vcs = worktrees_config.get('vcs', 'git')
        if vcs != 'git':
            print(f"VCS '{vcs}' is not supported (only 'git' is supported)")
            return None
            
        return worktrees_config
    
    def validate_and_read_template(self, worktrees_config: Dict[str, Any]) -> str:
        """
        ...
        """
        template_type = worktrees_config.get('template', 'default')
        template_file = worktrees_config.get('template_file', '')
        additional_files = worktrees_config.get('additional_files', [])
        additional_files_strategy = worktrees_config.get('additional_files_strategy', 'merge')
        ...
        if additional_files_strategy != 'replace' or not additional_files:
            if template_type == 'default':
                ...
                default_template_path = self.project_root / "ogc/control/commands/generic/worktrees.md"
    ...
    def generate_yaml_config(self, worktrees_config: Dict[str, Any], template_content: str) -> Dict[str, Any]:
        """Generate the YAML configuration matching the worktrees.yaml format."""
        return {
            ...
            "command": {
                "worktrees": {
                    ...
                    "agent": worktrees_config.get('agent', 'build'),
                    "model": worktrees_config.get('model', 'anthropic/claude-sonnet-4-20250514')
                }
            }
        }
    
    def write_yaml_config(self, worktrees_config: Dict[str, Any], template_content: str, output_path: str = "ogc/generated/.opencode/command/worktrees.md") -> None:
        ...
        description = worktrees_config.get('description', 'Worktrees command')
        agent = worktrees_config.get('agent', 'build')
        model = worktrees_config.get('model', 'anthropic/claude-sonnet-4-20250514')
...
{clean_content}"""
        ...
        print(f"Generated worktrees configuration (markdown): {output_file}")
    ...
    def generate(self) -> bool:
        ...
        try:
            ...
            # Validate worktrees configuration
            worktrees_config = self.validate_worktrees_config(config)
            if worktrees_config is None:
                return False
            
            # Validate template and read content
            template_content = self.validate_and_read_template(worktrees_config)
            
            # Write the configuration file in frontmatter format
            self.write_yaml_config(worktrees_config, template_content)
            
            return True
            
        except Exception as e:
            print(f"Error generating worktrees configuration: {e}", file=sys.stderr)
            sys.exit(1)
...
def main():
    ...
    generator = WorktreesConfigGenerator(config_path)
    success = generator.generate()
    
    if success:
        print("Worktrees configuration generated successfully")
    else:
        print("Worktrees configuration generation skipped")
