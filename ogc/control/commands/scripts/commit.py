...
commit command configuration based on the settings. It validates the configuration
and generates a JSON file matching the commit.jsonc template format.
"""
...
class CommitConfigGenerator:
    """Generates commit command configuration from TOML settings."""
    ...
    def validate_commit_config(self, config: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """
        Validate the commit configuration and return the commit settings.
    ...
    def validate_and_read_template(self, commit_config: Dict[str, Any]) -> str:
        ...
        if additional_files_strategy != 'replace' or not additional_files:
            if template_type == 'default':
                ...
                default_template_path = self.project_root / "ogc/control/commands/generic/git-commit.md"
    ...
    def generate_yaml_config(self, commit_config: Dict[str, Any], template_content: str) -> Dict[str, Any]:
        """Generate the YAML configuration matching the commit.yaml format."""
        return {
    ...
    def write_yaml_config(self, commit_config: Dict[str, Any], template_content: str, output_path: str = "ogc/generated/.opencode/command/commit.md") -> None:
